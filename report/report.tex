...
\noindent\colorbox{rwthblue25}{%
  ...
  speed from the wake frequency. The goal is a forward PDE solver used
  backwards as a measurement, with the finite-domain error reported. The
  core is numpy-only.\vspace{3pt}}}
...
measurement. The core is numpy-only.
...
For the lid-driven cavity, the centreline velocity profiles match the
Ghia et al.\ (1982) tabulated data to RMS of order $10^{-3}$ (Fig.~\ref{fig:bench}, left). The steady wake length and the VIV
lock-in amplitude validate the immersed body and the fluid--structure coupling against the
experimental literature.

\paragraph{Blockage.} A finite domain confines the flow, raising
...
\begin{figure}[h]
  ...
  \end{subfigure}
  \caption{Verification: the cavity benchmark (left) and the steady wake (right).}
  \label{fig:bench}
\end{figure}
